...
import { ElementEntry, SlotDefinition } from '../../registry';
...
// Slot definitions for HeroSection
export const heroSlotDefinitions: SlotDefinition[] = [
  {
    id: 'title',
    name: 'Hero Title',
    type: 'text',
  ...
  },
  {
    ...
    defaultValue: 'Create beautiful, responsive websites with our powerful design system. Start building your next project today.',
  ...
  },
  {
    ...
    type: 'node',
    required: false,
    description: 'Image, video, or illustration for the hero',
    placeholder: 'Add media content...',
  },
];
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  // Resolve content from slots or props (slots take precedence)
  const resolvedTitle = slots.title ?? title ?? 'Build Something Amazing';
  const resolvedSubtitle = slots.subtitle ?? subtitle ?? 'Create beautiful, responsive websites with our powerful design system. Start building your next project today.';
  const resolvedPrimaryCta = slots.primaryCta ?? ctaText ?? 'Get Started';
  const resolvedSecondaryCta = slots.secondaryCta ?? secondaryCtaText ?? 'Learn More';
  const resolvedMedia = slots.media;
...
// Slot definitions for FeatureGrid
export const featureGridSlotDefinitions: SlotDefinition[] = [
  {
    id: 'heading',
  ...
  },
  {
    ...
    defaultValue: 'Everything you need to build modern web applications',
    maxLength: 150,
  },
  {
    id: 'features',
...
interface FeatureGridProps {
  ...
  // Slot props
  slots?: Record<string, React.ReactNode | Array<{ icon: string; title: string; description: string }>>;
}
...
export const FeatureGrid: React.FC<FeatureGridProps> = ({
  ...
  // Resolve content from slots or props
  const resolvedHeading = (slots.heading as React.ReactNode) ?? 'Powerful Features';
  const resolvedSubheading = (slots.subheading as React.ReactNode) ?? 'Everything you need to build modern web applications';
  const resolvedFeatures = (slots.features as Array<{ icon: string; title: string; description: string }>) ?? features ?? defaultFeatures;
...
// Slot definitions for TestimonialsSection
export const testimonialsSlotDefinitions: SlotDefinition[] = [
  {
    id: 'heading',
    name: 'Section Heading',
    type: 'text',
...
interface TestimonialsSectionProps {
  ...
  // Slot props
  slots?: Record<string, React.ReactNode | Array<{ quote: string; author: string; role: string; avatar?: string }>>;
}
...
export const TestimonialsSection: React.FC<TestimonialsSectionProps> = ({
  ...
  // Resolve content from slots or props
  const resolvedHeading = (slots.heading as React.ReactNode) ?? 'What People Say';
  const resolvedTestimonials = (slots.testimonials as Array<{ quote: string; author: string; role: string; avatar?: string }>) ?? testimonials ?? defaultTestimonials;
...
// Slot definitions for CtaSection
export const ctaSlotDefinitions: SlotDefinition[] = [
  {
    id: 'title',
    name: 'CTA Title',
    type: 'text',
...
export const CtaSection: React.FC<CtaSectionProps> = ({
  ...
  // Resolve content from slots or props
  const resolvedTitle = slots.title ?? title ?? 'Ready to Get Started?';
  const resolvedSubtitle = slots.subtitle ?? subtitle ?? 'Join thousands of developers building with our design system.';
  const resolvedCta = slots.cta ?? ctaText ?? 'Start Free Trial';
...
// Slot definitions for Footer
export const footerSlotDefinitions: SlotDefinition[] = [
  {
    id: 'companyName',
    name: 'Company Name',
    type: 'text',
  ...
  },
  {
    ...
    defaultValue: 'Building the future of web development.',
    maxLength: 100,
  },
  {
    id: 'links',
    ...
    multiple: true,
    maxItems: 5,
    description: 'Groups of footer navigation links',
    defaultValue: [
...
interface FooterProps {
  ...
  showSocial?: boolean;
  // Slot props
  slots?: Record<string, React.ReactNode | Array<{ title: string; items: string[] }>>;
}
...
export const Footer: React.FC<FooterProps> = ({
  ...
  // Resolve content from slots or props
  const resolvedCompanyName = (slots.companyName as string) ?? companyName ?? 'Company';
  const resolvedTagline = (slots.tagline as React.ReactNode) ?? 'Building the future of web development.';
  const resolvedLinks = (slots.links as Array<{ title: string; items: string[] }>) ?? links ?? defaultLinks;
  const resolvedSocial = slots.social as React.ReactNode;
  const resolvedCopyright = (slots.copyright as React.ReactNode) ?? `© 2024 ${resolvedCompanyName}. All rights reserved.`;
  const bgOverrides = {
...
export const layoutRegistry: ElementEntry[] = [
  // Hero Section variants (with slots)
  createLayoutEntry('hero', 'Hero Section', 'Full-width hero with headline and CTA', ['hero', 'landing', 'header'], HeroSection, ['mol-button', 'mol-heading'], heroSlotDefinitions),
  createLayoutEntry('hero-dark', 'Hero Section Dark', 'Dark theme hero section', ['hero', 'dark', 'landing'], (props) => <HeroSection {...props} variant="dark" />, ['mol-button', 'mol-heading'], heroSlotDefinitions),
  createLayoutEntry('hero-brutal', 'Hero Section Brutal', 'Neo-brutal hero section', ['hero', 'brutal', 'landing'], (props) => <HeroSection {...props} variant="brutal" />, ['mol-button', 'mol-heading'], heroSlotDefinitions),
  createLayoutEntry('hero-neon', 'Hero Section Neon', 'Neon-styled hero section', ['hero', 'neon', 'landing'], (props) => <HeroSection {...props} variant="neon" />, ['mol-button', 'mol-heading'], heroSlotDefinitions),
  createLayoutEntry('hero-cosmic', 'Hero Section Cosmic', 'Cosmic gradient hero', ['hero', 'cosmic', 'landing'], (props) => <HeroSection {...props} variant="cosmic" />, ['mol-button', 'mol-heading'], heroSlotDefinitions),

  // Feature Grid variants (with slots)
  createLayoutEntry('feature-grid', 'Feature Grid', 'Grid of feature cards', ['features', 'grid', 'cards'], FeatureGrid, ['mol-card'], featureGridSlotDefinitions),
  createLayoutEntry('feature-grid-dark', 'Feature Grid Dark', 'Dark theme feature grid', ['features', 'dark'], (props) => <FeatureGrid {...props} variant="dark" />, ['mol-card'], featureGridSlotDefinitions),
  createLayoutEntry('feature-grid-glass', 'Feature Grid Glass', 'Glassmorphism features', ['features', 'glass'], (props) => <FeatureGrid {...props} variant="glass" />, ['mol-card'], featureGridSlotDefinitions),

  // Testimonials variants (with slots)
  createLayoutEntry('testimonials', 'Testimonials Section', 'Customer testimonials grid', ['testimonials', 'social-proof'], TestimonialsSection, ['mol-card', 'mol-avatar'], testimonialsSlotDefinitions),
  createLayoutEntry('testimonials-dark', 'Testimonials Dark', 'Dark theme testimonials', ['testimonials', 'dark'], (props) => <TestimonialsSection {...props} variant="dark" />, ['mol-card', 'mol-avatar'], testimonialsSlotDefinitions),
  createLayoutEntry('testimonials-cosmic', 'Testimonials Cosmic', 'Cosmic themed testimonials', ['testimonials', 'cosmic'], (props) => <TestimonialsSection {...props} variant="cosmic" />, ['mol-card', 'mol-avatar'], testimonialsSlotDefinitions),

  // CTA Section variants (with slots)
  createLayoutEntry('cta', 'CTA Section', 'Call-to-action banner', ['cta', 'banner', 'conversion'], CtaSection, ['mol-button', 'mol-heading'], ctaSlotDefinitions),
  createLayoutEntry('cta-brutal', 'CTA Section Brutal', 'Brutal-styled CTA', ['cta', 'brutal'], (props) => <CtaSection {...props} variant="brutal" />, ['mol-button', 'mol-heading'], ctaSlotDefinitions),
  createLayoutEntry('cta-neon', 'CTA Section Neon', 'Neon CTA banner', ['cta', 'neon'], (props) => <CtaSection {...props} variant="neon" />, ['mol-button', 'mol-heading'], ctaSlotDefinitions),

  // Footer variants (with slots)
  createLayoutEntry('footer', 'Footer', 'Site footer with links', ['footer', 'navigation'], Footer, ['mol-link-list'], footerSlotDefinitions),
  createLayoutEntry('footer-dark', 'Footer Dark', 'Dark theme footer', ['footer', 'dark'], (props) => <Footer {...props} variant="dark" />, ['mol-link-list'], footerSlotDefinitions),
  createLayoutEntry('footer-brutal', 'Footer Brutal', 'Neo-brutal footer', ['footer', 'brutal'], (props) => <Footer {...props} variant="brutal" />, ['mol-link-list'], footerSlotDefinitions),
