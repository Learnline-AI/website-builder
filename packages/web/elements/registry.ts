...
/**
 * SlotType defines what kind of content a slot accepts
 */
export type SlotType =
  | 'text'           // Plain text content
  | 'richText'       // Markdown or formatted text
  | 'node'           // React node (any renderable content)
  | 'component'      // A specific component type
  | 'list'           // Array of items
  | 'image'          // Image with src/alt
  | 'link'           // Link with href/label
  | 'action';        // Clickable action (button/link)

/**
 * SlotDefinition describes a customizable slot within a component
 */
export interface SlotDefinition {
  // Identity
  id: string;                          // Unique slot ID within component: 'title', 'cta', 'image'
  name: string;                        // Display name: 'Hero Title'

  // Content type
  type: SlotType;                      // What kind of content this slot accepts
  accepts?: string[];                  // For 'component' type: list of accepted component IDs

  // Configuration
  required: boolean;                   // Must be provided
  multiple?: boolean;                  // Can accept array of items (for 'list' type)
  maxItems?: number;                   // For 'list' type: maximum items

  // Default content
  defaultValue?: React.ReactNode | string | object;
  defaultComponent?: string;           // ID of default component to render

  // Documentation
  description: string;                 // What this slot is for
  placeholder?: string;                // Placeholder text for empty state

  // Constraints
  maxLength?: number;                  // For text types
  minLength?: number;                  // For text types
  allowedElements?: string[];          // For node type: allowed element IDs
}

/**
 * SlotContent represents the actual content provided to a slot
 */
export interface SlotContent {
  slotId: string;
  content: React.ReactNode | string | object;
  componentId?: string;                // If using a component
  props?: Record<string, unknown>;     // Props to pass to component
}

/**
 * Props for components that support slots
 */
...
export interface ElementEntry {
  ...
  // Slot system (for organisms/templates)
  slots?: SlotDefinition[];            // Customizable content slots
