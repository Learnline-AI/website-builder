...
export function ComponentCard({ component, zone, index, onClick }: ComponentCardProps) {
  ...
  return (
    <article
      ...
      aria-label={`${component.name}: ${component.description}. ${component.isInteractive ? 'Interactive component.' : ''} Press Enter to view details.`}
      className={`
      ...
      <button
        ...
        className={`
          ...
          ${isFavorite(component.id)
            ...
            : 'bg-black/30 text-white/70 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 focus:opacity-100 focus:scale-100'
          }
          hover:scale-110
        `}
      >
